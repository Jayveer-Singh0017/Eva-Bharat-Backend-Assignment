...
**Live app:** [https://media-sequencer-xtb5.onrender.com](https://eva-bharat-backend-assignment-2.onrender.com/)
**API:** https://media-sequencer-api.onrender.com — health: `/health`, full state: `/api/state`

Author: Jayveer Singh — [[GitHub](https://github.com/Jayveer-Singh0017) · [LinkedIn](https://www.linkedin.com/in/jayveer-singh-13493a2a6/)
...
The maths lives in `backend/src/main/java/com/mediasequencer/schedule/ScheduleService.java` as pure, unit-tested Java, and is
...
Java 21 + Spring Boot 3 · Spring Data JPA / Hibernate · PostgreSQL (pgx wire-compatible) / H2 fallback ·
Server-Sent Events · React (Vite) · Docker · deployed on Render.

## Run locally

Backend — optional `DATABASE_URL` in environment or `backend/.env` (defaults to embedded H2 if not set):
...
Docker: `cd backend && docker build -t media-sequencer-api . && docker run -p 8080:8080 media-sequencer-api`
